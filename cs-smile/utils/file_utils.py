...
class FileUtils:
    ...
    def get_python_files(path: str) -> list[str]:
        """
        Retrieves all Python files from the specified path, ignoring virtual environments.
        ...
        """
        result = []
        ignored_dirs = {".venv", "venv", "lib"}  # Set di cartelle da ignorare

        if os.path.isfile(path) and path.endswith(".py"):
            return [path]

        for root, dirs, files in os.walk(path):
            dirs[:] = [d for d in dirs if d not in ignored_dirs]  # Filtriamo le cartelle da ignorare
            for file in files:
                if file.endswith(".py"):
